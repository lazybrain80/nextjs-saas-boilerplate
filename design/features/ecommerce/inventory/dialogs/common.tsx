      ...
      <CardContent className="space-y-6">
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <Input defaultValue={inventoryItem?.title} onChange={e => setTitle(e.target.value)} />
          </div>
          <div className="space-y-2">
...
export const ProductVariations = () => {
  ...
  return (
    <Card className="rounded-3xl">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">{'Variation'}</CardTitle>
      </CardHeader>
            ...
            <div key={variation.id} className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    setVariartions(
                      inventoryItem?.variations.filter(
                        (v: ProductVariation) => v.id !== variation.id
                      ) || []
                    )
                  }}
                >
              ...
              <Input
                ...
                onChange={e => {
                  ...
                  setVariartions(
                    (inventoryItem?.variations ?? []).map((v: ProductVariation) => {
...
export const ProductPricing = () => {
  const { inventoryItem, setPrice, setDiscount, setDiscountType, setTaxRate, setTaxType } =
    useInventory()

  return (
    <Card className="rounded-3xl">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">{'Pricing'}</CardTitle>
      </CardHeader>
      <CardContent>
    ...
    <Card className="rounded-3xl">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>{'Status'}</span>
          <Icons.Circle
            className={cn('h-4 w-4', statusColors[inventoryItem?.status as InventoryStatus])}
          />
        </CardTitle>
      </CardHeader>
      <CardContent>
...
export const ProductDetails = () => {
  ...
  return (
    <Card className="rounded-3xl">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">{'Product Details'}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex items-center space-x-4">
...
export const ProductThumbnails = () => {
  ...
  return (
    <Card className="rounded-3xl">
      <CardHeader>
        <CardTitle className="flex items-center justify-between">{'Thumbnail'}</CardTitle>
      </CardHeader>
      <CardContent>
        <FileImporter
        ...
        <div className="space-y-2">
          {(inventoryItem?.thumbnails ?? []).map(thumb => (
            <div key={thumb} className="flex items-center space-x-2">
              <Image src="/images/ecommerce/file_icon.svg" alt="thumbnail" width={24} height={24} />
              <span>{thumb}</span>
            </div>
          ))}
        </div>
