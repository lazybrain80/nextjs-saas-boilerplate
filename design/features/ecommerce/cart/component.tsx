...
import { Drawer, Button, Separator, Transition } from '@/design/components'
...
const ShoppingCartHeader = () => {
  ...
  return (
    <div className={cn('flex items-center justify-between p-2 space-x-4')}>
      <span className="text-black text-2xl font-bold">Shopping Cart</span>
      <Button
        className={cn(
...
const ShoppingCartBody = () => {
  ...
  return (
    <div className={cn('flex flex-col p-2 space-y-2', 'overflow-y-auto')}>
      {cartItems.length > 0 ? (
        <>
          {cartItems.map((item, index) => (
            <div key={index} className={cn('flex items-center my-2')}>
              <Image
              ...
              <div className="w-1/2 ml-4 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <Button className="w-6 h-6" onClick={() => minusProduct(item.id)}>
                      -
                    </Button>
                    <span> {item.quantity} </span>
                    <Button className="w-6 h-6" onClick={() => plusProduct(item.id)}>
                      +
                    </Button>
                  </div>
                </div>
        ...
        <Transition className="flex-col items-center justify-center p-4 space-y-4" direction="down">
          <Image
            className="w-full"
