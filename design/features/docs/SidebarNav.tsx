...
const iconMap = new Map([['home', Icons.Home]])
  ...
  return items.length ? (
    <div className="w-full">
      {items.map(item => {
        ...
        return (
          <div key={item.href + item.title} className={cn('mb-1')}>
            <Link
              key={item.title + item.href}
              href={item.href ? item.href : ''}
              className={cn('flex w-full rounded-md hover:bg-slate-200', {
            ...
            >
              ...
              >
                ...
                <span className="rounded-md px-2 py-1 text-sm font-bold ">{item.title}</span>
              </span>
            </Link>
      ...
      {items.map(item =>
        !item.disabled && item.href ? (
          <Link
            key={item.title + item.href}
            href={item.href}
            className={cn('flex w-full items-center rounded-md p-2 hover:bg-slate-200', {
              'bg-muted': pathname === item.href,
            })}
