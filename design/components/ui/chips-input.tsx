...
export const ChipsInput: React.FC<ChipsInputProps> = ({
  ...
  const handleChipAdd: HandleChipAdd = useCallback((chipValue) => {
      ...
      >
        <div className='flex flex-wrap gap-2'>
          {chips.map((chip, index) => (
            <div
              key={index}
              className='flex items-center gap-1 bg-blue-100 text-blue-800 px-3 py-1 rounded-full group hover:bg-blue-200 transition-colors'
              role='button'
              tabIndex={0}
            >
